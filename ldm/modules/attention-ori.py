...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        sim = sim.softmax(dim=-1)                       # attention_map

        out = einsum('b i j, b j d -> b i d', sim, v)   # cross-attention output (feature update)
        out = rearrange(out, '(b h) n d -> b n (h d)', h=h)
        print('cross-attention output:', out.shape)
        print('cross-attention output to_out:', self.to_out(out).shape)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            print('context[i]:', context[i].shape) # torch.Size([b, 77, 768])
            print('x:', x.shape)                   # torch.Size([b, 4096, 128])
        if self.use_linear:
            x = self.proj_out(x)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if not self.use_linear:
            x = self.proj_out(x)
        print('spatial transformer, x:', x.shape)
        print('spatial transformer, x_in:', x_in.shape)
        return x + x_in
